...
function ServicesPageContent() {
  ...
  const fetchServices = React.useCallback(async () => {
    const normalizedBranchId = activeBranchId === "ALL_BRANCHES" ? undefined : (activeBranchId ?? undefined);
    console.debug("[services-page] fetch:start", {
      brandSlug,
      activeBranchId,
    ...
    try {
      ...
      console.debug("[services-page] fetch:result", {
        servicesResult,
        roleResult,
      });
      if (servicesResult.success) {
        console.debug("[services-page] state:set-services", {
          count: servicesResult.data.length,
          sample: servicesResult.data[0] ?? null,
        });
        setServices(servicesResult.data);
      } else {
        console.warn("[services-page] fetch:services-error", servicesResult.error);
      }
      if (roleResult.success) {
        setUserRole(roleResult.data.role);
      } else {
        console.warn("[services-page] fetch:role-error", roleResult.error);
      }
    } catch (err) {
  ...
  React.useEffect(() => {
    const handleRefresh = () => {
      console.debug("[services-page] event:seervis:services-refresh");
      fetchServices();
    };
  ...
  React.useEffect(() => {
    console.debug("[services-page] filter:result", {
  ...
  }, [services.length, filteredServices.length, statusFilter, technicianFilter, search, activeBranchId]);
  ...
  const handleServiceUpdated = useCallback(() => {
    const normalizedBranchId = activeBranchId === "ALL_BRANCHES" ? undefined : (activeBranchId ?? undefined);
    listServicesAction({ brandSlug, branchId: normalizedBranchId }).then((result) => {
  ...
  }, [brandSlug, selectedServiceId, activeBranchId]);
          ...
          <AnimatePresence mode="wait" initial={false} custom={viewDirection}>
            {viewMode === "list" ? (
              <motion.div
                ...
                className="w-full"
              >
                <ServiceListView services={filteredServices} role={userRole ?? undefined} />
              </motion.div>
            ) : (
              <motion.div
                key="kanban"
