-- ============================================================
-- Migration 034: Fix POS V4 Checkout Payment Resolution
--
-- checkout_pos_v4 was using resolve_pos_payment_account which
-- queries payment_methods by id. But POS V4 sends
-- branch_payment_methods.id as p_payment_method_id, so the
-- lookup in payment_methods fails.
--
-- Fix: resolve payment account directly from
-- branch_payment_methods by id, validating brand, branch,
-- active status, and linked payment account.
--
-- Also drops FK constraint on pos_transactions.payment_method_id
-- since V4 stores branch_payment_methods.id (not payment_methods.id).
-- ============================================================

alter table if exists public.pos_transactions
  drop constraint if exists pos_transactions_payment_method_id_fkey;
...
comment on function public.checkout_pos_v4 is
  'Atomic POS V4 checkout. Resolves payment account from branch_payment_methods. Processes quantity items and Unit Second serialized items.';
