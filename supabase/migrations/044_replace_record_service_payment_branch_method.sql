-- ============================================================
-- Migration 044: Replace record_service_payment for branch_payment_methods
--
-- Replaces the active database function definitively.
-- p_payment_method_id is branch_payment_methods.id, not payment_methods.id.
-- ============================================================
...
begin
  ...
  -- p_payment_method_id is legacy parameter name.
  -- In current service payment flow, its value is branch_payment_methods.id.
  select
    bpm.id,
    bpm.method_type,
  ...
  if v_method_type = 'CASH' then
    -- For CASH: use bpm account if set, otherwise fallback to branch default cash account
    ...
    -- Non-CASH: require bpm.payment_account_id is not null
  ...
  -- Step 10: Insert service_payment record
