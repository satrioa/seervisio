-- ============================================================
-- Migration 140: Revert POS V4 payment resolution regression
--
-- checkout_pos_v4 was redefined in migration 098 (SECURITY
-- DEFINER) using resolve_pos_payment_account, which looks up
-- public.payment_methods by id. POS V4 sends a
-- branch_payment_methods.id, so that lookup always fails with:
--   "Payment method <id> not found for brand <n>"
-- and checkout aborts before any ledger/stock work happens.
--
-- Fix: restore the correct resolution (migrations 034/038/091)
-- directly from public.branch_payment_methods by id, while
-- keeping SECURITY DEFINER (098) and the reference_id uuid fix
-- (091). Also restores mdr_min_transaction handling.
-- ============================================================
...
begin
  ...
  -- 3. Generate transaction number (SECURITY DEFINER -- bypasses RLS on counter table)
  ...
  loop
    ...
    if v_item_type in ('PRODUCT_QUANTITY', 'UNIT_NEW_QUANTITY') then
      ...
      insert into public.inv_stock_movements (
        ...
        'POS_TRANSACTION', v_transaction_id, v_transaction_number,
...
comment on function public.checkout_pos_v4 is
  'Atomic POS V4 checkout (SECURITY DEFINER). Resolves payment account from branch_payment_methods by id. Processes quantity items and Unit Second serialized items.';
