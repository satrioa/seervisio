-- ============================================================
-- Migration 046: Add defensive payment number retry
--
-- Problem: If a previous transaction (that called
-- generate_service_payment_number) rolled back, the counter
-- state may give the same number again, causing a UQ violation.
-- Also, idempotency key from TypeScript uses Date.now(), so
-- retries produce different keys that don't match the pre-check.
--
-- Fix: Wrap the INSERT in a retry loop. On uq_sp_payment_number
-- violation, generate a fresh number and retry once.
-- ============================================================
...
begin
  ...
  -- Step 8: Generate payment number (atomic via SELECT FOR UPDATE)
  <<retry_block>>
  loop
    v_payment_number := public.generate_service_payment_number(v_service.brand_id);

    -- Step 9: Auto-generate idempotency key if not provided
    if v_final_key is null then
      v_final_key := 'service_payment:' || p_service_id || ':' || v_payment_number;
    end if;

    -- Step 10: Insert service_payment record (with defensive retry)
    begin
    ...
    exception
      when unique_violation then
        ...
        -- Reset auto-generated key so a fresh one is created on retry
...
comment on function public.record_service_payment is
  'Records a service payment: validates branch_payment_methods, checks idempotency first, resolves account, calculates MDR (with min threshold), generates number atomically with defensive retry, creates payment + account movement in one transaction.';
