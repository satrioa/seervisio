    ...
    <div className="space-y-4">
      ...
      <div className="overflow-hidden rounded-lg border bg-card">
        ...
        <div className="grid grid-cols-[1fr_120px_100px_80px_120px_80px_40px] gap-2 border-b bg-muted/50 px-3 py-2 text-[10px] font-medium uppercase tracking-wider text-muted-foreground">
          ...
          >
            Servis
            {sortField === "totalServices" && (
              sortDir === "asc" ? <ChevronUp className="size-3" /> : <ChevronDown className="size-3" />
            )}
          </button>
          <button
            type="button"
            onClick={() => handleSort("lastServiceAt")}
            className="flex items-center gap-1 text-left"
          >
            Terakhir
            {sortField === "lastServiceAt" && (
              sortDir === "asc" ? <ChevronUp className="size-3" /> : <ChevronDown className="size-3" />
            )}
          </button>
        ...
        {filteredCustomers.length === 0 ? (
          ...
          paginatedCustomers.map((customer) => (
            <button
              ...
              className="grid w-full grid-cols-[1fr_120px_100px_80px_120px_80px_40px] gap-2 border-b px-3 py-2.5 text-left transition-colors last:border-0 hover:bg-muted/20"
            >
              {/* Customer Name */}
              ...
              {/* Total Spend */}
              <div className="flex items-center justify-end text-xs font-medium tabular-nums text-foreground">
              ...
              <div className="flex items-center text-[10px] text-muted-foreground">
                {customer.lastServiceAt
                  ? new Date(customer.lastServiceAt).toLocaleDateString("id-ID", {
                      day: "numeric",
                      month: "short",
                      year: "numeric",
                    })
              ...
              <div className="flex items-center justify-center">
                {customer.activeWarranties > 0 ? (
                  <Badge
                    variant="outline"
                    className="border-green-200 bg-green-50 text-[9px] text-green-700 dark:border-green-800 dark:bg-green-950 dark:text-green-400"
                  >
                    {customer.activeWarranties}
                  </Badge>
                ) : (
              ...
              {/* Arrow */}
              <div className="flex items-center justify-center">
                <ChevronDown className="size-3.5 -rotate-90 text-muted-foreground/50" />
              </div>
            </button>
          ))
        )}
