...
const ITEM_TYPE_OPTIONS = [
  ...
  { value: "CONSUMABLE", label: "Konsumsi" },
  { value: "PRODUCT", label: "Produk" },
  { value: "DEVICE_UNIT", label: "Unit Second" },
...
export default function InventoryPage() {
  ...
  React.useEffect(() => {
    getInventoryCategoriesAction(window.location.pathname.split("/")[1]).then((res) => {
  ...
  React.useEffect(() => {
    setPage(1);
  }, [search, branchFilter, categoryFilter, typeFilter, trackingFilter, statusFilter]);
  ...
  }, [branchFilter, categoryFilter, typeFilter, trackingFilter, statusFilter, search, page, pageSize, activeBranchId, refreshKey]);
  ...
  React.useEffect(() => {
    if (activeTab === "unit-second") fetchSerializedUnits();
  }, [activeTab, fetchSerializedUnits]);
  ...
  const unitSecondCount = data?.items.filter((i) => i.itemType === "DEVICE_UNIT").length ?? 0;
    ...
    <div className="space-y-4">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="h-auto gap-0 border-b bg-transparent p-0">
          <TabsTrigger value="items" className="relative rounded-none border-b-2 border-transparent bg-transparent px-4 py-2.5 text-xs font-medium text-muted-foreground shadow-none transition-colors data-[state=active]:border-foreground data-[state=active]:text-foreground">
            <Package className="mr-1.5 size-3.5" /> Item Stok
          </TabsTrigger>
          ...
          <TabsTrigger value="unit-second" disabled className="relative rounded-none border-b-2 border-transparent bg-transparent px-4 py-2.5 text-xs font-medium text-muted-foreground shadow-none transition-colors">
            <Boxes className="mr-1.5 size-3.5" /> Unit Second
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="items" className="mt-4 space-y-4">
          <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-6">
            ...
            <SummaryCard label="Unit Second" value={loading ? "..." : String(unitSecondCount)} icon={Boxes} helper="siap jual" />
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Select value={typeFilter} onValueChange={setTypeFilter}>
              <SelectTrigger className="h-9 w-[110px] text-xs"><SelectValue placeholder="Tipe" /></SelectTrigger>
              <SelectContent>{ITEM_TYPE_OPTIONS.map((o) => (<SelectItem key={o.value} value={o.value} className="text-xs">{o.label}</SelectItem>))}</SelectContent>
            </Select>
            ...
            {canManage && (
              <>
                ...
                <Button size="sm" variant="outline" className="h-9 text-xs" onClick={() => { setAdjustDialogOpen(true); }}>
        ...
        <TabsContent value="unit-second" className="mt-4">
          <div className="mb-3 flex flex-wrap items-center gap-2">
            ...
            {canManage && (
              <Button size="sm" className="h-9 gap-1.5 text-xs" onClick={() => { setEditSerializedUnit(null); setSuFormOpen(true); }}>
      ...
      <StockAdjustmentDialog
        ...
        onOpenChange={() => { setAdjustDialogOpen(false); handleRefreshMovements(); }}
