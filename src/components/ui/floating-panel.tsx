      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className={cn(
              "fixed z-50 overflow-hidden border border-border bg-card text-card-foreground shadow-lg outline-none",
              className
            )}
            style={{
              borderRadius: 12,
...
interface FloatingPanelFormProps {
  children: React.ReactNode
  onSubmit?: (note: string) => void | boolean | Promise<void | boolean>
  className?: string
}
