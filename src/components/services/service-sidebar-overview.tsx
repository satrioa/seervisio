...
export function ServiceSidebarOverview() {
  ...
  const trendData = React.useMemo(() => generateTrendData(), []);

  const stats = React.useMemo(() => {
    const today = new Date().toISOString().slice(0, 10);
    const todayServices = services.filter((s) => s.createdAt.startsWith(today));
    return {
      totalMasuk: services.filter((s) => s.status === "masuk").length,
      dalamPerbaikan: services.filter((s) => s.status === "perbaikan" || s.status === "diagnosa").length,
      qc: services.filter((s) => s.status === "qc").length,
      selesaiHariIni: todayServices.filter((s) => s.status === "selesai").length,
    };
  }, [services]);
            ...
            <div className="rounded-lg border bg-card p-3">
              <ChartContainer config={chartConfig} className="h-[200px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={trendData} margin={{ top: 5, right: 8, bottom: 5, left: -20 }}>
